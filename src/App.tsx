      ...
      {Object.keys(scores).map((score) => (
          <div key={score} className='text-xl my-6'>
            <label>
              ...
              {score.replace(/([A-Z])/g, ' $1').charAt(0).toUpperCase() + score.replace(/([A-Z])/g, ' $1').slice(1)}:
              <select className='border' name={score} value={scores[score as keyof Scores].value} onChange={handleChange}>
